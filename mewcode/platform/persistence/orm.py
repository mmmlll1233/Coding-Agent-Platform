...
class NotificationOutboxRow(Base):
    ...
    __table_args__ = (
        UniqueConstraint("job_id", "source_event_sequence", "destination"),
        CheckConstraint(
...
class WorkerNodeRow(Base):
    ...
    service_type: Mapped[str] = mapped_column(
        String(32), server_default="worker", nullable=False
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "service_type IN ('worker', 'notifier')", name="service_type_values"
        ),
        Index("ix_worker_nodes_service_heartbeat", "service_type", "heartbeat_at"),
    )
